...
class ToolsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.closing = Form.objects.create(title="Closing Procedures", slug='closing-procedures', status=STATUS_PUBLISHED)
        self.closing.sites.add(self._site)
        self.closing.save()
        self.printer = Form.objects.create(title="Printer Issues", slug='printer-issue', status=STATUS_PUBLISHED)
        self.printer.sites.add(self._site)
        self.printer.save()
        self.login = Form.objects.create(title="Login Issues", slug='login-issue', status=STATUS_PUBLISHED)
    ...
    def testToolsExist(self):
        ...
        response = c.get('/tools/closing-procedures/')
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Closing Procedures")
        response2 = c.get('/tools/printer-issue/')
        self.assertEqual(response2.status_code, 200)
        self.assertContains(response2, "Printer Issues")
        response3 = c.get('/tools/login-issue/')
